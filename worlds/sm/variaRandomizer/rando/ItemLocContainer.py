...
class ContainerSoftBackup(object):
    ...
    def restore(self, container, resetSM=True):
        ...
        if resetSM:
            container.settlementManager.resetItems()
            container.settlementManager.addItems([it.Type for it in container.currentItems])
...
class ItemLocContainer(object):
    ...
    def transferCollected(self, dest):
        ...
        dest.settlementManager = SMBoolManager(self.sm.player, self.sm.maxDiff, self.sm.onlyBossLeft)
        dest.settlementManager.addItems([item.Type for item in dest.currentItems])
